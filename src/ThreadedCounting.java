...
public class ThreadedCounting {
    ...
    public static void countInThreads(Counter counter) {
        new ThreadedCounting(counter).doCount();
    }

    private volatile int count = 1;
    private final Counter counter;
    public ThreadedCounting(Counter counter) {
    ...
    private void doCount() {
        ...
        for (int i = 0; i < 3; ++i)
            (threads[i] = new Thread(new CountingRunnable(i))).start();
        try {
    ...
    private class CountingRunnable implements Runnable {
        ...
        public CountingRunnable(int offset) {
            this.offset = offset;
        }
        @Override
        public void run() {
            for (; count <= 100; Thread.yield())
                synchronized (ThreadedCounting.this) {
                    if (count % 3 == offset) {
                //        counter.count(count++);
                    }
                }
